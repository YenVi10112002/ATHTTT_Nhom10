...
import utils
from tourapp import app, login, conn
...
# @app.route('/pay_history/<int:user_id>/<int:bill_id>')
# def delete_bill(bill_id, user_id):
#     user_id = current_user.id
#     utils.delete_bill(bill_id=bill_id)
#     return redirect(url_for('pay_history', user_id=user_id))

@app.route('/pay_history/<int:user_id>/<int:bill_id>', methods=['DELETE'])
def delete_bill(bill_id, user_id):
    if conn.execute("DELETE FROM bill WHERE bill_id=?", bill_id):
        conn.commit()
        conn.close()
        return redirect(url_for('pay_history', user_id=user_id))
    return redirect(url_for('home'))
